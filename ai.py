...
class Guesser:
    ...
    def nextGuess(self, choices):
        if VERB:
            print("inG choices:", choices)
        temp = self.preprocess(self.clues[0][0].lower())
        if VERB:
            print("inG preclue:", self.clues[0], temp, self.isKnown(temp))
...
class Spymaster:
    ...
    class Combo:
        def __init__(self):
            self.scores = (
                []
            )  # all similarity scores gen'd for this combo, regardless of clue
    ...
    def makeClue(self, board, blue):
        ...
        if VERB:
            print("mH max_combo:", max_combo)  # DEBUG
...
if __name__ == "__main__":
    ...
    board = {
        ...
        "R": ["cat", "button", "pipe", "pants", "mount", "sleep", "stick", "file"],
